...
public class DAOCustomerDemographics extends DBConnection {
    ...
    public int updateCusDemoGrap(CustomerDemographics other) {
        ...
        int n = 0;
        String sql = "DELETE FROM CustomerDemographic WHERE CustomerID = ?";

        try {
            PreparedStatement pre = conn.prepareStatement(sql);
            pre.setString(1, customerTypeID);  // Use setString to handle the CustomerID (since it's nvarchar)
...
public Vector<CustomerDemographics> getCustomers (String sql){
        ...
        try {
            ...
            while(re.next()){
                ...
                String CustomerDesc = re.getString("CustomerDesc");
